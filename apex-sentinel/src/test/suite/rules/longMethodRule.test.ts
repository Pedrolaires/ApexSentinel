...
describe('LongMethodRule — Unit and Integration tests', () => {
  ...
  describe('Unit tests (manual metrics)', () => {
    it('detects method with LOC > threshold', () => {
      ...
      const ctx = createContext(metrics, { enabled: true, threshold: 20 } as RuleConfig);
    ...
    it('detects method with NOP > threshold', () => {
      ...
      const ctx = createContext(metrics, { enabled: true, nopThreshold: 5 } as RuleConfig);
      const rule = new LongMethodRule();
      const res = rule.apply(ctx);
      assert.strictEqual(res.length, 1);
    ...
    it('detects method with CC > threshold', () => {
      ...
      const ctx = createContext(metrics, { enabled: true, ccThreshold: 10 } as RuleConfig);
      const rule = new LongMethodRule();
      const res = rule.apply(ctx);
      assert.strictEqual(res.length, 1);
  ...
  describe('Integration tests (AST from fixtures → MetricVisitor → Rule)', () => {
    it('reports for fixture tooLong.cls', () => {
      const metrics = getMetricsFromFixture('tooLong.cls');
      const ctx = createContext(metrics, { enabled: true, threshold: 20 } as RuleConfig);
      const rule = new LongMethodRule();
      const res = rule.apply(ctx);
    ...
    it('reports for manyParams.cls', () => {
      const metrics = getMetricsFromFixture('manyParams.cls');
      const ctx = createContext(metrics, { enabled: true, nopThreshold: 5 } as RuleConfig);
      const rule = new LongMethodRule();
      const res = rule.apply(ctx);
    ...
    it('reports for complexMethod.cls (cc)', () => {
      const metrics = getMetricsFromFixture('complexMethod.cls');
      const ctx = createContext(metrics, { enabled: true, ccThreshold: 5 } as RuleConfig);
      const rule = new LongMethodRule();
      const res = rule.apply(ctx);
    ...
    it('no report for ok.cls', () => {
      const metrics = getMetricsFromFixture('ok.cls');
      const ctx = createContext(metrics, { enabled: true } as RuleConfig);
      const rule = new LongMethodRule();
      const res = rule.apply(ctx);
